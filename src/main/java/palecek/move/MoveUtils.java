...
public class MoveUtils {
    ...
    public static List<MoveComponent> parseMove(String move, boolean isOffset) {
        List<MoveComponent> moveComponents = new ArrayList<>();
        if (isOffset) {
        ...
        completeMove(moveComponents);
        return moveComponents;
    }

    private static void completeMove(List<MoveComponent> moveComponents) {
        ...
        if (moveComponents.getFirst().getDirection().isInLine(moveComponents.get(1).getDirection())) {
            throw new IllegalArgumentException("Invalid move format (Directions are in line): " + moveComponents.getFirst().getDirection() + " and " + moveComponents.get(1).getDirection());
        }
    }

    public static Set<Position> calculateExpectedPositions(Position from, List<MoveComponent> move, Orientation orientation, Board board) {
    ...
    private static void fillPositions(Set<Position> positions, MoveComponent line1, MoveComponent line2, Position from, Orientation orientation, Board board) {
     ...
     */
    private static void repete(Position pos, int modulo1, int modulo2, Set<Position> positions, int maxX, int maxY, Direction direction1, Direction direction2) {
        int x = pos.getX();
        int y = pos.getY();
